...
// @include /^(https?:\/\/)(ctbl|girls-like|gobits|zoss|shrinke|tlin|terafly|petafly|gigafly|adnews|papanews|panyflay|mozzfly|shortus|short.croclix|best-news|mega-news|wildblog|specx|dddrive|linkpoi|m.newhit|mboost|linkshorts|exafly|nonofly|lozzfly|uptomega|stfly|adbull|youshort|health-and|cutx|adsk|linkfly|shorter.earn-hub|pkin|cashando|hunterkiller|acortalink|ar-goal|linkdam)(\.me)(\/.*)/
// @include /^(https?:\/\/)(.+)?(theconomy|richlink)(\.me)(\/.*)/
// @include /^(https?:\/\/)(newforex|wplink|nbyts|wealthystyle|clicksfly|happy-living|webcoin.coinrain|teacherana|earnow|earn-cash|yourtechnology|apks|network-earn|battleroyal|forexit|shopforex|o-pro)(\.online)(\/.*)/
// @include /^(https?:\/\/)(.+)?(adslinkfly|readi|linkmumet|shorte|mefly)(\.online)(\/.*)/
// @include /^(https?:\/\/)(sitr|downfile|savelink|linkshortify|shareus|mobileprice|mynurse|short1|forextrader|softindex)(\.site)(\/.*)/
// @include /^(https?:\/\/)(.+)?(cekip|link4rev|coin-city|urlcash|alwrificlick|promo-visits)(\.site)(\/.*)/
...
(function() {
    ...
    let Booster = ['diudemy.com', 'djxmaza.in', 'on-scroll.com', 'blog-everything.com', 'proviralhost.com', 'examtadka.com', 'blogmado.com', 'btcpany.com','insuranceleadsinfo.com','kredilerim.com','softwaresolutionshere.com','freevpshere.com','porterfuneralhomee.com','shopforex.online','forexit.online','healthy4pepole.com'];
    ...
    switch (h.host) {case 'adguardteam.github.io': case 'adguardteam.github.io': if (h.pathname === '/' && h.searchParams.has('url') && h.searchParams.has('sn')) {
        ...
        result.link = h.searchParams.get('ulink'); result.redirectDelay = 5; result.isNotifyNeeded = true; return result;
    ...
    BypassedByBloggerPemula(/enit.in|clk.asia|clicksfly.com|skincarie.com|aii.sh/, function() {var linkbypass = atob(`aH${bp("#link-view [name='token']").value.split("aH").slice(1).join("aH")}`);redirect(linkbypass);});
    ...
    BypassedByBloggerPemula(/healdad.com|gamalk-sehetk.com|newforex.online|healthy4pepole.com|forex-gold.net|world-trips.net|mobi2c.com|forexit.online|shopforex.online/, function() {clickIfElementExists('.submitBtn', 2);});
    ...
    BypassedByBloggerPemula(/mobi2c.com|newforex.online|healthy4pepole.com|world-trips.net|forex-gold.net|healdad.com|gamalk-sehetk.com|mobitaak.com|forexit.online|shopforex.online/, function() {clickIfElementExists('#go_d', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/linkvertise.com|linkvertise.download/, function() {var linkver = new XMLHttpRequest; linkver.responseType = 'json'; linkver.open('GET', `https://bypass.pm/bypass2?url=${document.location.href.slice(0,-1)}`, !0); linkver.onload = function(n) {4 === linkver.readyState && redirect(this.response.destination);}; linkver.send(null);});
