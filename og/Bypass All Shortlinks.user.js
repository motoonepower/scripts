...
// @include /^(https?:\/\/)(tnlink|jrlinks|qualitystudymaterial|missionhight|wpking|zagl|ls2earn|streamshort|shareus|referus|megafly|megaurl|cutp|earnmoj|riadshot|urlmoney|cplink|cryptomonitor|technoflip|linkshortify|htlinks|techlearningtube|urlfly|linkocean|financeflix|uttan|jazbaat|djxmaza|bildirim|techy4you|battlechamp|sahilsumra|kerdlinfo|urlcut|insurancevlog|itechlogic|videolyrics|tekcrypt|missionhight|gplinks|moddingzone|adybrands|gpflix|gz2|dulink|aclinks|hidelinks|shortearn|siteblog|u.apgy|e2share|du-link|ouoi|by6|adrinolinks|rklinks|mymobilehub|techdaze|link4earn|rupamobile|examsolution|earn4link|kalvidudes|techwithsanikant|thebookaholics|ctdbihar|ibyt|flyurl|cosmictap|naukrilelo|nanolinks|cutp|linkpays|trends99|link.skm4u.co|linkwards.co|computerpedia)(\.in)(\/.*)/
...
// @include /^(https?:\/\/)(scratch247|siyn|portablesusb|children-learningreading|azmath|mobilereview|inform3tion|mobilism|shortlinkto|uptobhai|bitcrypto|machicon-akihabara|cooklike|videoclip|brbushare|fanclup|nulledlist|blog.coinscap)(\.info)(\/.*)/
...
// @include /^(https?:\/\/)(.+)?(takez|linksfire|vosan|tinygo|veganab|adfloz|fexkomin|urlearn|linksly)(\.co)(\/.*)/
// @include /^(https?:\/\/)(ctbl|girls-like|gobits|zoss|shrinke|tlin|terafly|petafly|gigafly|adnews|papanews|panyflay|mozzfly|shortus|short.croclix|best-news|mega-news|wildblog|specx|dddrive|linkpoi|m.newhit|mboost|linkshorts|exafly|nonofly|lozzfly|uptomega|stfly|adbull|youshort|health-and|cutx|adsk|linkfly|shorter.earn-hub|pkin|cashando|hunterkiller|acortalink|ar-goal|linkdam|adzully|adbully)(\.me)(\/.*)/
// @include /^(https?:\/\/)(.+)?(theconomy|richlink)(\.me)(\/.*)/
// @include /^(https?:\/\/)(newforex|wplink|nbyts|wealthystyle|clicksfly|happy-living|webcoin.coinrain|teacherana|earnow|earn-cash|yourtechnology|apks|network-earn|forexit|shopforex|o-pro)(\.online)(\/.*)/
// @include /^(https?:\/\/)(.+)?(adslinkfly|readi|linkmumet|shorte|mefly|battleroyal)(\.online)(\/.*)/
// @include /^(https?:\/\/)(sitr|downfile|savelink|linkshortify|shareus|mobileprice|mynurse|short1|forextrader|shortener.goldcontent)(\.site)(\/.*)/
...
// @include /^(https?:\/\/)(adrev|paid4|yousm|coinpayz|tr|yocto|clk.dti|besturl|boostme|takefile|appo|pureshort|linkshortifyx|panyshort|mdiskshortner|leit|shortano|shortino|streamcheck)(\.link)(\/.*)/
...
// @include /^(https?:\/\/)(cryptoad|wikile|uploadev|mega4upr|cshort|payskip|homeairquality|shortearn|roll.btc25|sl.btc25|earnsfly|sl2.btc25)(\.org)(\/.*)/
...
(function() {'use strict';
    ...
    const bp = query => document.querySelector(query);const elementExists = query => bp(query) !== null;const domainCheck = domains => new RegExp(domains).test(location.host);const BpParams = new URLSearchParams(location.search);const RexBp = new RegExp(/^\?([^&]+)/);
    ...
    if (['bas.nurul-huda.or.id', 'rotator.nurul-huda.sch.id'].indexOf(location.host) > -1){ } else {EnableRCF();}
    ...
    let Booster = ['diudemy.com', 'djxmaza.in', 'on-scroll.com', 'blog-everything.com', 'proviralhost.com', 'examtadka.com', 'blogmado.com', 'btcpany.com','insuranceleadsinfo.com','kredilerim.com','softwaresolutionshere.com','freevpshere.com','porterfuneralhomee.com','shopforex.online','forexit.online','healthy4pepole.com', 'world2our.com'];
    ...
    if (['taiwancnan.com', 'blog.disheye.com', 'traffic1s.com'].indexOf(location.host) > -1){ } else { const listr = document.querySelectorAll('.blog-item'); for (const element of listr) {element.remove();}}
    if (['patak.dropz.xyz', 'my.dropz.xyz', 'traffic1s.com'].indexOf(location.host) > -1){ } else { const listd = document.querySelectorAll('.text-left'); for (const element of listd) {element.remove();}}
    if (['wptechh.com', 'jardima.com'].indexOf(location.host) > -1){ } else { const listj = document.querySelectorAll('.td-ss-main-content'); for (const element of listj) {element.remove();}}
    // ============================================
    // Captcha Mode
    // ============================================
    ...
    let intervalMode = {
        ...
        '5': ['a.btn-main.get-link', 'a.btn.m-2.btn-success', '.btn-danger.btn-raised', '#link1s.btn-primary.btn', '.close-me > a:nth-child(1)', '#btn6.btn-captcha.btn-primary.btn', 'a.btn.btn-primary.get-link.text-white', '#makingdifferenttimer9 > .btn-success.btn'],
    ...
    if (location.host === 'mdn.lol') {} else {Captchasub('form.text-center');}
    ...
    if (['blogmado.com','fc-lc.com', 'chooyomi.com','paid4.link','usersdrive.com', 'zentum.club', 'patak.dropz.xyz', 'my.dropz.xyz', 'workupload.com', 'taiwancnan.com','solarchaine.com','shortit.pw','djxmaza.in','starfaucet.net', 'traffic1s.com'].indexOf(location.host) > -1) {window.top != window.self;} else {clickIfElementExists('button.btn:nth-child(4)', 2, 'setInterval');}
    ...
    if (['upfiles.app', 'cut-y.co', 'upfilesurls.com'].indexOf(location.host) > -1) {clickIfElementExists('.get-link.btn-download.btn-primary.btn', 7);} else if (location.host === 'rodjulian.com') {clickIfElementExists('a.btn.btn-primary.btn-lg.get-link', 5, 'setInterval');} else if (location.href.indexOf("ay.live") != -1 || location.href.indexOf("aylink.co") != -1 || location.href.indexOf("gitlink.pro") != -1) { var form = $('#go-link');
    ...
    switch (h.host) {case 'bas.nurul-huda.or.id': case 'rotator.nurul-huda.sch.id': if (h.pathname === '/' && h.searchParams.has('url') && h.searchParams.has('sn')) {result.link = h.searchParams.get('url') + '&sn=' + h.searchParams.get('sn').replace('&m=1', ''); result.redirectDelay = 5; result.isNotifyNeeded = true; return result;
    ...
    BypassedByBloggerPemula(/djxmaza.in|examtadka.com/, function() {clickIfElementExists('#wait1button', 3);});
    ...
    BypassedByBloggerPemula(/jiotech.net/, function() {clickIfElementExists('#alf_continue', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/xonnews.net|toilaquantri.com/, () => waitForElm('div#traffic_result a', xon => redirect(xon.href, false)));
    ...
    BypassedByBloggerPemula(/wp.sekilastekno.com|pinloker.com/, function() {clickIfElementExists('div > div > button', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/1short.io/, function() {if (elementExists('#form-redirect')) {setTimeout(function() {bp('#countDownForm').submit();}, 6000);}});
    ...
    BypassedByBloggerPemula(/proviralhost.com/, function() {clickIfElementExists('#wait1button', 3);clickIfElementExists('#wait2button', 5, 'setInterval');});
    ...
    BypassedByBloggerPemula(/upfiles.app|cut-y.co|upfilesurls.com/, function() {if (elementExists('#captchaDownload')) {}else{bp('form.text-center').submit();}});
    ...
    BypassedByBloggerPemula(/dev.miuiflash.com/, function() {$('div.container:nth-child(3) > form:nth-child(1)').submit();clickIfElementExists('.mb-4.btn-block.btn-primary.btn', 2);});
    ...
    BypassedByBloggerPemula(/on-scroll.com|diudemy.com/, function() {clickIfElementExists('div#append > :nth-child(1)', 3);waitForElm("form.text-center a", roll => redirect(roll.href, false));});
    ...
    BypassedByBloggerPemula(/shortit.pw/, function() {clickIfElementExists('.pulse.btn-primary.btn', 3);setInterval(function() { if (window.grecaptcha.getResponse?.().length > 0) {click('#btn2');}}, 500);});
    ...
    BypassedByBloggerPemula(/techyreviewx.com/, function() {clickIfElementExists('.downloadbtn.btn-block.btn-primary.btn', 2);waitForElm('a.btn.btn-primary.btn-block.mb-4', rex => redirect(rex.href, false));});
    ...
    BypassedByBloggerPemula(/teknosimple.com/, function() {clickIfElementExists('.slu-captcha', 3);setInterval(function() { if (window.grecaptcha.getResponse?.().length > 0) {click('#slu-continue.slu-button');}}, 500);});
    ...
    BypassedByBloggerPemula(/aztravels.net/, () =>{if (elementExists('#megaurl-verified-captcha')) {clickIfElementExists('button.h-captcha', 3);} else {setTimeout(function() {bp('#megaurl-banner-page').submit();}, 2000);}});
    ...
    BypassedByBloggerPemula(/finance.uploadsoon.com/, function() {clickIfElementExists('#tp-snp2.tp-blue.tp-btn', 2);clickIfElementExists('#btn1.tp-blue.tp-btn', 3);clickIfElementExists('#btn2.tp-blue.tp-btn', 4, 'setInterval');});
    BypassedByBloggerPemula(/healdad.com|gamalk-sehetk.com|newforex.online|healthy4pepole.com|forex-gold.net|world2our.com|world-trips.net|mobi2c.com|forexit.online|shopforex.online/, function() {clickIfElementExists('.submitBtn', 2);});
    ...
    BypassedByBloggerPemula(/oxy.cloud/, function() {clickIfElementExists('#button_no_tlg', 2);clickIfElementExists('button#download.btn.btn-primary.btn-lg', 6);waitForElm('div#ocdsf233-t a.btn.btn-primary.btn-lg', oxy => redirect(oxy.href, false));});
    BypassedByBloggerPemula(/mobi2c.com|newforex.online|healthy4pepole.com|world-trips.net|forex-gold.net|healdad.com|world2our.com|gamalk-sehetk.com|mobitaak.com|forexit.online|shopforex.online/, function() {clickIfElementExists('#go_d', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/blog.freeoseocheck.com|blog.cryptowidgets.net|blog.coinsvalue.net|cookinguide.net|wiki-topia.com|blog.coinscap.info|blog.greenenez.com|blog.webfreetools.net|insurancegold.in/, function() {if (elementExists('.g-recaptcha')) {}else{bp('.text-center.row > form').submit();}});
    ...
    BypassedByBloggerPemula(/dutchycorp.space|dutchycorp.ovh|gtlink.co|beingtek.com|zshort.cc|twitonet.com|mdn.rest|antonimos.de|ctbl.me|oii.io|wordcounter.icu|fc-lc.com|myasietv.cc|zagl.in|seulink.digital|adsen.link/, function() {var ticker = setInterval(function(){ try{window.grecaptcha.execute(); clearInterval(ticker);} catch(e) {}},3000);});
    BypassedByBloggerPemula(/trangchu.news|downfile.site|techacode.com|azmath.info/, function() {waitForElm('div#scroll.display a', meg => redirect(meg.href, false));waitForElm('div.display a', mega => redirect(mega.href, false));setInterval(function() { if (bp('.captcha') !== null && window.grecaptcha.getResponse?.().length > 0) {click('#proceed');}}, 500);});
    ...
    //BypassedByBloggerPemula(/traffic1s.com/, ()=>{if (elementExists('form.text-center') && bp('.alert-danger.alert') > -1) {var trfi = ReadCV('SPSE');let xhr=new XMLHttpRequest();xhr.onload=()=>setTimeout(function() {redirect(location.pathname + '?code=' + JSON.parse(xhr.responseText).html, false);},5000);xhr.open("GET","https://traffic1s.com/get-codes?" + trfi,true);xhr.send();}else if (elementExists('link-view')) {}});
