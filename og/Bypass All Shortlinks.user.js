...
(function() {'use strict';
    ...
    if (['webhostingpost.com', 'stfly.me'].indexOf(location.host) > -1) {} else {Captchasub("form[action='']");}
    ...
    BypassedByBloggerPemula(/satoshi-win.xyz|shorterall.com|promo-visits.site/, function() {ClickIfExists('.box-main > button:nth-child(3)', 1);});
    ...
    BypassedByBloggerPemula(/jiotech.net|automat.systems/, function() {const jiot = BpAll('.mg-headwidget,.col-md-3,text,footer,.mb-4.p-3,.mg-header,.mg-headwidget,iframe,.wp-post-image'); for (const element of jiot) {element.remove();}ClickIfExists('#alf_continue', 3, 'setInterval');});
    ...
    BypassedByBloggerPemula(/oko.sh|ckk.ai|adsy.pw|linksfy.co|techgeek.digital|short.express|cryptosh.pro|goo.st|playstore.pw|shortzon.com|insurancegold.in|mitly.us|fc-lc.com|cashando.me|yoshare.net|linka.click|adsy.pw|linkpay.cc|tinycmd.com|tinygo.co|urlw.net|exee.app|exeo.app|ufacw.com|pndx.live|makemoneywithurl.com|forex-trnd.com|gawbne.com|sharemods.com/, function(){if(elementExists('.g-recaptcha')||elementExists('#captchaShortlink')){}else{SubmitIfExists('#link-view',1)||SubmitIfExists('#form-continue',1)||SubmitIfExists('.col-md- > form',1)||SubmitIfExists('#nextstepform',1)||SubmitIfExists('.text-center.row > form',1)||SubmitIfExists('div.col-md-12 form',1)||SubmitIfExists('div.text-center form',1)||SubmitIfExists('#submit-form',1)||SubmitIfExists('#before-captcha',1)||SubmitIfExists('#yuidea',1)||SubmitIfExists('#dForm',1)||SubmitIfExists('#exfoary-form',1)||SubmitIfExists('#hidden form',1)||SubmitIfExists('#submit_first_page',1);}});
