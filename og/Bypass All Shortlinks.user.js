...
// @include /^(https?:\/\/)(girls-like|gobits|zoss|shrinke|tlin|terafly|petafly|gigafly|adnews|papanews|panyflay|mozzfly|shortus|short.croclix|best-news|mega-news|wildblog|dddrive|linkpoi|m.newhit|mboost|linkshorts|exafly|nonofly|lozzfly|uptomega|stfly|adbull|youshort|health-and|cutx|adsk|linkfly|shorter.earn-hub|pkin|cashando|hunterkiller|acortalink|ar-goal|linkdam|adzully|adbully|zumpa|botfly|weezo)(\.me)(\/.*)/
...
// @exclude /^https:\/\/faucet.tribuncrypto\.com\/(dashboard|advertise|ptc|deposit|transfer|tasks|offerwall|profile|auto|mining|dice|lottery|faucet|login)/
// @exclude *://*.vosan.co/
...
(function() {'use strict';
    ...
    function Checkvisibility(elem) {if(!elem.offsetHeight && !elem.offsetWidth) { return false; }if(getComputedStyle(elem).visibility === 'hidden') { return false; } return true;}
    ...
    function Captchasub(query, act = 'submit', timeInSec = 0.5) {if (elementExists(query)) {var timer = setInterval(function() {if (Captchacheck()) {bp(query)[act](); clearInterval(timer);}}, timeInSec * 1000);}}
    ...
    if (['bas.nurul-huda.or.id', 'rotator.nurul-huda.sch.id', 'autofaucet.dutchycorp.space'].indexOf(location.host) > -1){ } else {EnableRCF();}
    ...
    if (location.host === 'zshort.cc' && location.search === '') {location = 'https://antonimos.de/?url8j=' + location.href;}
    if (['uploadhaven.com', 'patak.dropz.xyz', 'my.dropz.xyz'].indexOf(location.host) > -1) {} else {ClickIfExists('button.btn.btn-success', 7);}
    if (['webhostingpost.com', 'stfly.me', 'coins-town.com', 'autofaucet.dutchycorp.space'].indexOf(location.host) > -1) {} else {Captchasub("form[action='']");}
    ...
    BypassedByBloggerPemula(/shortit.pw/, function() {ClickIfExists('.pulse.btn-primary.btn', 3);setInterval(function() { if (Captchacheck()) {click('#btn2');}}, 500);});
    ...
    BypassedByBloggerPemula(/computerpedia.in/, function() {ClickIfExists('.tp-blue.tp-btn-2', 3);setInterval(function() { if (Captchacheck()) {click('#tp-snp2');}}, 500);});
    ...
    BypassedByBloggerPemula(/teknosimple.com/, function() {ClickIfExists('.slu-captcha', 3);setInterval(function() { if (Captchacheck()) {click('#slu-continue.slu-button');}}, 500);});
    ...
    BypassedByBloggerPemula(/trangchu.news|downfile.site|techacode.com|azmath.info|aztravels.net/, function() {waitForElm('div#scroll.display a', meg => redirect(meg.href, false));waitForElm('div.display a', mega => redirect(mega.href, false));setInterval(function() { if (Captchacheck() && bp('.captcha') !== null) {ClickIfExists('#proceed', 1);}}, 500);});
    ...
    BypassedByBloggerPemula(/dutchycorp.space|dutchycorp.ovh|gtlink.co|beingtek.com|zshort.cc|twitonet.com|antonimos.de|oii.io|wordcounter.icu|myasietv.cc|zagl.in|seulink.digital|adsen.link|flyad.vip|cutty.app/, function() {if(elementExists('.grecaptcha-badge')||elementExists('#captchaShortlink')){var ticker = setInterval(function(){ try{window.grecaptcha.execute(); clearInterval(ticker);} catch(e) {}},3000);}});
